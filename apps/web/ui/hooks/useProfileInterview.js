...
export function useProfileInterview() {
  ...
  useEffect(() => { readApi("/profile/conversation-facts").then(value => {setConfirmedFacts(value.facts); setProfileLoaded(true);}).catch(() => setReviewNotice("Saved profile details could not be loaded. Check the local services before confirming.")); }, []);
  ...
  async function saveConfirmedFacts() {
    if (!profileLoaded) { setReviewNotice("Wait for the saved profile to load before confirming."); return; }
    ...
    try {
      const saved = await readApi("/profile/conversation-facts", {method: "PUT", body: JSON.stringify({confirmed: true, facts: next.map(({field,value,evidence}) => ({field,value,evidence}))})});
      setConfirmedFacts(saved.facts);
    } catch (error) {
      setReviewNotice("The details were not saved. " + error.message);
      return;
    }
    const remaining = {
      ...pendingProfile,
    ...
    setReviewNotice(
      `${chosen.length} reviewed detail${chosen.length === 1 ? "" : "s"} saved to your profile.`,
    );
    addMessage({
      role: "assistant",
      kind: "profile-confirmed",
      text: `You confirmed ${chosen.length} profile detail${chosen.length === 1 ? "" : "s"}. They are saved in your local CareerOS database.`,
    });
